  ...
  (match p
     ...
     (if (empty? ps)
         (let ([ok (gensym)])
           (list (seq (seq)
                 ...
                 cm))
         (compile-pattern (Cons (car ps) (List (cdr ps))) cm next))]
    ;; TODO
    ;; This code just pops and goes to next clause.
    ;; Replace with code that implements pattern.
    [(Vect ps)
     (if (empty? ps)
         (let ([ok (gensym)])
           (list (seq (seq)
                 ...
                 cm))
         (match (letrec ([match-vect-els (lambda (ps cm)
                                           (match ps
                                             ['() (list (seq) cm)]
                                             [(cons p ps) (match (compile-pattern p (cons #f cm) next)
                                                            [(list i1 cm1)
                                                             (match (match-vect-els ps cm1)
                                                               [(list i2 cm2)
                                                                (list (seq (seq)
                                                                           ;; save rax on stack
                                                                           (Push rax)
                                                                      ...
                                                                      cm2)])])]))])
                  (match-vect-els ps cm))
           [(list i cm1)
            (let ([done (gensym)]
                  [not-ok (gensym)])
              (list (seq (seq)
                         ;; ensure rax holds a vector and untag
                         (Mov r8 rax)
                         (And r8 ptr-mask)
                         (Cmp r8 type-vect)
                         (Jne not-ok)
                         ...
                         ;; if rax holds empty vector, we are notok
                         (Cmp rax 0)
                         (Je not-ok)

                         ;; get vect length into r8
                         (Mov r8 (Offset rax 0))
                         ;; make sure vect length = ps length
                         (Cmp r8 (length ps))
                         (Jne not-ok)

                         ;; move rax up a word, pointing it to first el
                         (Add rax 8)

                         ;; match els against ps
                         i
                         (Jmp done)
    ...
    [(Pred f)
     (let ([ok (gensym)]
           [r (gensym)])
       (list (seq (seq)
                  ;; put return addr into r8
                  (Lea r8 r)
                  ;; push r8 onto stack
                  (Push r8)

                  ;; rax holds the value being matched
                  ;; push rax onto stack as an arg
                  (Push rax)

                  ;; jmp to (symbol->label f)
                  (Jmp (symbol->label f))

                  ;; Label r
                  (Label r)

                  ;; if rax not false, ok; else, next
                  ;; compare rax to (val->bits #f)
                  (Cmp rax (value->bits #f))
                  ;; if not equal, we have a truthy result so jump to ok
                  (Jne ok)
                  ;; jump to next
                  (Add rsp (* 8 (length cm)))
                  (Jmp next)
